...
def pandas_to_supervised(dataset:pd.DataFrame,converter=None,**converter_params) -> dict:
    """ Parse a pandas dataframe and convert it to a list of supervised timeseries evaluation datasets, 
    one dataset per split key. As converter a dataframe to supervised converter should be used. """
    if converter is not None:
        dataset = converter(dataset,**converter_params)
    return dataset

def json_to_supervised(dataset:dict,converter=None,**converter_params) -> dict:
    """ Parse a json dict and convert it to a list of supervised timeseries evaluation datasets, 
    one dataset per split key. As converter a dataframe to supervised converter should be used. """
    dataset_pd = pd.DataFrame.from_dict(dataset)
    if converter is not None:
        dataset_final = converter(dataset_pd,**converter_params)
