...
from .data_handler import DataHandler, Preprocessor, Analyser
from .models import LinearModel, ClassifierModel
from .utils import timer, test_stationary, printProgressBar
...
def atm_supply_analysis():
    ...
    analysis.to_csv(f'{AtmoConfiguration.results_conf['results_path']}/load_stats.csv',sep=';',decimal=',',quotechar='"',index=False)
    analysis = atmomore_analyser.timeseries_statistics()
    analysis.to_csv(f'{AtmoConfiguration.results_conf['results_path']}/load_timeseries_stats.csv',sep=';',decimal=',',quotechar='"',index=False)
    ...
    analysis.to_csv(f'{AtmoConfiguration.results_conf['results_path']}/load_timeseries_stats_filled.csv',sep=';',decimal=',',quotechar='"',index=False)
...
def train_models(grouped_timeseries=None,time_granularity=None,algorithm=0):
    ...
    for atm in grouped_timeseries:
        try:
            ...
            continue
    pandas.DataFrame.from_records(stationary_tests).to_csv(f'{AtmoConfiguration.results_conf['results_path']}/stationary.csv',sep=';',decimal=',',quotechar='"')
...
def evaluate_models_timelag(algorithm,train_timeseries,test_timeseries,lag_days=1) -> pandas.DataFrame:
    ...
    for atm in common_atms:
        ...
        Path(f"{AtmoConfiguration.results_conf['results_path']}/plots/{atm}/full").mkdir(parents=True, exist_ok=True)          
        ...
        plt.grid(True)
        plt.savefig(f'{AtmoConfiguration.results_conf['results_path']}/plots/{atm}/full/plot_{algorithm}_{atm}_full.png')
        plt.close()
    ...
    for atm in models:
        try:
            ...
            for test in data_tests:  
                Path(f"{AtmoConfiguration.results_conf['results_path']}/plots/{atm}/{test}").mkdir(parents=True, exist_ok=True)          
                ...
                plt.legend(loc="lower right") 
                plt.grid(True)
                plt.savefig(f'{AtmoConfiguration.results_conf['results_path']}/plots/{atm}/{test}/plot_{algorithm}_{atm}_{test}.png')
                plt.close()
                for metric in metrics:
                    try:
                        y_pred_copy = [metrics[metric].get('converter',float)(score) for score in y_pred]
...
def evaluate_lstm(timeseries,lag_days=1):
    ...
    results = []
    Path(f"{AtmoConfiguration.results_conf['results_path']}/figures/lstm").mkdir(parents=True, exist_ok=True)  
    for atm in common_atms:
        train_dataset = train_dataset_dict[atm]
        test_dataset = test_dataset_dict[atm]
        ...
        #plt.grid(True)
        #plt.savefig(f'{AtmoConfiguration.results_conf['results_path']}/figures/lstm/lstm_training_{atm}_{lag_days}.png')
        #plt.close()
