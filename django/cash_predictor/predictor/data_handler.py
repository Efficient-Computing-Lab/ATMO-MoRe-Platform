...
from cash_predictor.predictor.utils import printProgressBar, resampler, clear_duplicate_timestamps, timer
...
class DataHandler():
    ...
    def mongo(self) ->pymongo.MongoClient:
        client = pymongo.MongoClient(**AtmoConfiguration.mongo_conf)
        return client[AtmoConfiguration.datastore_conf['db']]
    ...
    def import_supply_info(self,filepath=None,atm_info=None) -> list:
        ...
        if filepath is None:
            filepath = os.path.join(AtmoConfiguration.datastore_conf['datastore_path'],AtmoConfiguration.datastore_conf['datastore_supply_path'])
    ...
    def import_test_supply_info(self,filepath=None,atm_info=None) -> list:
        """ Reads atm supply info from file and inserts it into mongo """
        if filepath is None:
            filepath = os.path.join(AtmoConfiguration.datastore_conf['datastore_path'],AtmoConfiguration.datastore_conf['datastore_supply_test_path'])
        supply_info = pandas.ExcelFile(filepath).parse(sheet_name='Sheet1').to_dict(orient='records')
        for record in supply_info:
            if isinstance(record['Date'],str):
                record['Date'] = datetime.strptime(record['Date'], "%d/%m/%Y")
    ...
    def import_outage_info(self,filepath=None,atm_info=None) -> list:
        ...
        if filepath is None:
            filepath = os.path.join(AtmoConfiguration.datastore_conf['datastore_path'],AtmoConfiguration.datastore_conf['datastore_outage_path'])
        outage_info = []
        filepaths = []
    ...
    def get_atm_info(self,atm_list:list=None) -> dict:
        if atm_list is None:
    ...
    def get_cit_info(self,cit_list:list=None) -> dict:
        if cit_list is None:
            return list(self.mongo['cit_info'].aggregate([]))
        else:
            return list(self.mongo['cit_info'].aggregate([
                {
                    "$match":{
    ...
    def cit_to_city(self, cit:str) -> str:
        city = cit.split(' ')[-1].upper()
        if city.endswith('Σ'):
            city = city[:-1]
        elif city.endswith('ΩΝ'):
            city = city[:-2] + "Α"
        return city
    
    def fill_cit_info(self): 
        gn = geocoders.GeoNames(username="vpsomak")
        atm_info = self.get_atm_info()
        cit_set = {}
        cit_info = []
        for atm in atm_info:
            cit_set[atm['cit_local']] = 1
        for cit in cit_set:
            city = self.cit_to_city(cit)
            try:
                gcode = gn.geocode(f"{city}",country="GR")
                cit_info.append({
                    '_id':cit,
                    'lat':gcode[1][0],
                    'lon':gcode[1][1],
                ...
                print(f"ERROR: {city}")
        self.mongo.drop_collection('cit_info')
        self.mongo['cit_info'].insert_many(cit_info)
        return cit_info
    ...
    def atm_renew_data(self):
        ...
        try:
            ...
            results['supply_info'] = {"success":True}
        except Exception as ex:
            print(f"Data import failed: {ex}")
            results['supply_info'] = {"success":False,"error":str(ex)}
        try:
            self.import_test_supply_info()
            results['test_supply_info'] = {"success":True}
        except Exception as ex:
            print(f"Data import failed: {ex}")
            results['test_supply_info'] = {"success":False,"error":str(ex)}
        try:
            self.import_outage_info()
            results['outage'] = {"success":True}
        except Exception as ex:
            print(f"Data import failed: {ex}")
            results['outage'] = {"success":False,"error":str(ex)}
        try:
            self.fill_cit_info()
            results['cit_info'] = {"success":True}
        except Exception as ex:
            print(f"Data import failed: {ex}")
...
class Preprocessor():
    ...
    def create_load_timeseries(self,skip:int=0,size:int=0,coverage_threshold:float=0.7,samples_threshold:int=20,atm_codes:list=None) -> list:
        """ Creates the load timeseries per ATM on daily basis after applying the thresholds and pre-processing """
        dataset = self.apply_samples_threshold(threshold=samples_threshold,data=self.clean_data)
    ...
    def test_to_supervised(simple_data:pandas.DataFrame,train_features:list) -> dict:
        ...
        simple_data = simple_data.groupby(['ATM', 'date'], as_index=False).agg({
            'value': 'mean',
            **{col: 'first' for col in simple_data.columns if col not in ['date','value','ATM']}
        })
    ...
    def initiate_evaluation_datasets():
        ...
        timeseries_test = atmomore_preprocessor_test.create_load_timeseries(coverage_threshold=0.0,samples_threshold=2)
...
class Analyser():
    ...
    def __init__(self,dataset=None):
        if dataset != None:
            ...
            atmomore_preprocessor = Preprocessor(supply_info)
            atmomore_preprocessor.clean_supply_types()
            self.dataset = atmomore_preprocessor.clean_data
    ...
    def atm_statistics(self,atm_code:str) -> pandas.DataFrame:
        """ Get historical data about the specified ATM """
        atmomore_preprocessor = Preprocessor(self.dataset)
        atmomore_preprocessor.clean_data = self.dataset
        timeseries = atmomore_preprocessor.create_load_timeseries(coverage_threshold=0.0,samples_threshold=2)
        grouped_timeseries = Preprocessor.group_by('ATM',timeseries)
        if atm_code in grouped_timeseries:
            return pandas.DataFrame().from_records(grouped_timeseries[atm_code])
        else:
